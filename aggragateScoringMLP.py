                ...
                for n_early_stopping_epochs in range(3, 8):
                    for batch_size in [pow(2, 7 + i) for i in range(5)]:
                        ...
                        for i in range(len(dictsForTraining)):
                            ...
                            precision, recall, thresholds = precision_recall_curve(y_cv, yhat_cv)
                            pr_auc = auc(recall, precision)
...
saveAsPickle(allArchitecturesAucs,
             os.path.join(path.aggregateFunctionMLPDir, os.path.join('gridSearch', 'allArchitecturesAucs')))
saveAsPickle(totalAucs, os.path.join(path.aggregateFunctionMLPDir, os.path.join('gridSearch', 'totalAucs')))
